## Application of the Refined Alarmism Dictionary to the Blind Corpus
...
df_raw <- read_xlsx("data/blind_corpus.xlsx")
...
v1_grupoA <- c("dana", "riada", "inundacion\\w*", "tormenta", "temporal", "victimas", "dan\\w*", "destruccion", "lluvias", "precipitacion\\w*", "litro\\w*", "gota fria")
...
audit_with_evidence <- function(txt, tokens, expres, gA, gB) {
  if (is.na(txt) | txt == "") return(list(valor = 0, Evidence = "Absence"))
  t_clean <- clean_text(txt)

  # Sorting by length to avoid false positives
  ...
  for (e in expres) {
    if (str_detect(t_clean, e)) return(list(valor = 1, Evidence = paste("Expresion:", e)))
  }

  # Evaluation for tokens
  for (tok in tokens) {
    if (str_detect(t_clean, paste0("\\b", tok, "\\b"))) return(list(valor = 1, Evidence = paste("Token:", tok)))
  }
  ...
  if (!is.na(match_A) & !is.na(match_B)) {
    return(list(valor = 1, Evidence = paste("Patron:", match_A, "+", match_B)))
  }

  return(list(valor = 0, Evidence = "Absence"))
}
...
df_final <- df_raw %>%
  mutate(
    V1 = map_int(res_v1, ~.x$valor), Evidence_V1 = map_chr(res_v1, ~.x$Evidence),
    V2 = map_int(res_v2, ~.x$valor), Evidence_V2 = map_chr(res_v2, ~.x$Evidence),
    V3 = map_int(res_v3, ~.x$valor), Evidence_V3 = map_chr(res_v3, ~.x$Evidence),
    V4 = map_int(res_v4, ~.x$valor), Evidence_V4 = map_chr(res_v4, ~.x$Evidence)
  )

# 6. Prevalence analysis

n_corpus <- nrow(df_final)
...
cat("Blind corpus size:", n_corpus, "headlines\n\n")
print(prevalences)

# 7. File exporting
write.table(
  df_final,
  file      = "dict_validation_blind_data.csv",
  sep       = ";",
  row.names = FALSE,
  col.names = TRUE
)
